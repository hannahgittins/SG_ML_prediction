"""Training gnn model with augmented coordinates on multiple gpus."""
...
class Trainer:
    """Trainer for the augemented data GNN."""
    ...
    def __init__(
        ...
        train_data :torch_geometric.loader.DataLoader
            training dataseet
        val_data :torch_geometric.loader.DataLoader
            validation dataset
        lr : float
            learning rate
        criterion : object
        ...
        if os.path.isfile(ckp_path) and os.path.isfile(best_path):
            ...
            self.start_epoch = self.initial_epoch
    ...
    def _random_translation(self):
        translations = [
            ...
            0.0,
            0.0,
        ]  ## higher likelyhood of 0 over other translations
    ...
    def _random_angle(self):
        return round(180 * random.random())

    def _aug_batch_dataset(self, dataset):
        rotations = [
            R.from_euler("z", self._random_angle(), degrees=True),
            R.from_euler("y", self._random_angle(), degrees=True),
            R.from_euler("x", self._random_angle(), degrees=True),
        ]
        ...
        for data in dataset:
            ...
            new_data = data
            other_attr_x = data.x[:, 0:5].cpu()
            coord = data.x[:, 4:-1].cpu().numpy()
            for rotate in rotations:
                new_coord = rotate.apply(coord)
                new_data.x = torch.cat(
                    (
                        torch.Tensor(other_attr_x).to(torch.float32),
                        torch.Tensor(new_coord).to(torch.float32),
                    ),
                    1,
                )
    ...
    def _run_batch(self, data: object, epoch: int):
        self.optimizer.zero_grad()

        data = self._aug_batch_dataset(data)
...
def main(
    ...
    # print(f"Example -> edge_index {example.edge_index.size()}, x {example.x.size()}, edge_attr {example.edge_attr.size()}")
    model = MP_model(
...
if __name__ == "__main__":
    ...
    classic_dropout = 0
    best_path = (
        f"/scratch/hs9g19/GNN_store/aug_data/best_gnn_model_ddp_test_aug_{temp}.pt"
    )
    ckp_path = (
        f"/scratch/hs9g19/GNN_store/aug_data/current_model_dpp_test_aug_{temp}.pt"
    )
    target_dict_file = "/scratch/hs9g19/GNN_store/datasets/All_organics_opt_molecular_graphs_coord_label_dict.pickle"
